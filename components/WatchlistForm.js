  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (isEditing) {
        ...
        // Delete old checklist items and re-insert
      ...
      if (checklist.length > 0) {
        const checkItems = checklist.map((label, i) => ({
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-[#111118] border border-[#2a2a3a] rounded-2xl shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {STRATEGY_TAGS.map((tag) => (
                <button
                  key={tag}
                  onClick={() => setStrategyTag(strategyTag === tag ? '' : tag)}
                  className={`px-3 py-1.5 rounded-lg text-xs transition-all ${
                    strategyTag === tag
                      ? 'bg-purple-500/20 text-purple-400 ring-1 ring-purple-500/30'
                      : 'bg-[#1c1c28] text-zinc-500 hover:text-zinc-300'
                  }`}
                >
                  {tag}
                </button>
            ...
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <input type="number" value={entryPrice} onChange={(e) => setEntryPrice(e.target.value)} placeholder="5000" className="w-full bg-[#1c1c28] border border-[#2a2a3a] rounded-lg py-2 px-3 text-sm text-white placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-emerald-500/40 font-mono" />
                  </div>
                  <div>
                    <div className="text-[10px] text-zinc-500 mb-1">Stop Loss</div>
                    <input type="number" value={slPrice} onChange={(e) => setSlPrice(e.target.value)} placeholder="4750" className="w-full bg-[#1c1c28] border border-[#2a2a3a] rounded-lg py-2 px-3 text-sm text-red-400 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-red-500/40 font-mono" />
                  </div>
                  <div>
                    <div className="text-[10px] text-zinc-500 mb-1">Take Profit</div>
                    <input type="number" value={tpPrice} onChange={(e) => setTpPrice(e.target.value)} placeholder="5500" className="w-full bg-[#1c1c28] border border-[#2a2a3a] rounded-lg py-2 px-3 text-sm text-emerald-400 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-emerald-500/40 font-mono" />
                  </div>
                </div>
              </div>

              {/* Entry Checklist Builder */}
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors text-sm"
                  >
                    +
                  </button>
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Catatan thesis, alasan, timing..."
                  rows={3}
          ...
          {/* Error */}
          {error && (
            <div className="text-sm text-red-400 bg-red-500/[0.04] border border-red-500/20 rounded-lg px-3 py-2">
              {error}
            </div>
          )}
          ...
          {!isEditing && (
            <p className="text-[11px] text-zinc-600 text-center">
              Entry, SL, TP, checklist bisa diisi nanti lewat tombol edit
            </p>
          )}
        </div>
      </div>
